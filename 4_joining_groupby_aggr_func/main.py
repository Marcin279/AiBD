...
def film_in_category(category_id:int)->pd.DataFrame:
    ...
    if not isinstance(category_id, int):
        ...
        querry: str = ("SELECT film.title, language.name AS languge, category.name AS category FROM film " + 
               ...
               "ORDER BY film.title, languge").format(c=category_id)
...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    if not isinstance(category_id, int):
        ...
        querry: str = ("SELECT cat.name AS category, COUNT(film_category.film_id) FROM category cat " + 
               "INNER JOIN film_category ON cat.category_id = film_category.category_id " + 
               "WHERE film_category.category_id = {cat_id} " + 
               "GROUP BY cat.name").format(cat_id=category_id)
...
def number_film_by_length(min_length: Union[int,float] = 0, max_length: Union[int,float] = 1e6 ) :
    ...
    if (not isinstance(min_length, (int, float))) or (not isinstance(max_length, (int, float))):
        ...
        querry: str = ("SELECT film.length, COUNT(film.film_id) FROM film " + 
               "WHERE film.length BETWEEN {min_l} AND {max_l} " + 
               "GROUP BY film.length").format(min_l=min_length, max_l=max_length)
...
def client_from_city(city:str)->pd.DataFrame:
    ...
    if not isinstance(city, str):
        ...
        querry: str = ("SELECT city.city, customer.first_name, customer.last_name FROM customer " +
               "INNER JOIN address ON customer.address_id = address.address_id " +
               "INNER JOIN city ON address.city_id = city.city_id "
               "WHERE city.city = '{c}' ").format(c=city)
...
def avg_amount_by_length(length:Union[int,float])->pd.DataFrame:
    ...
    if not isinstance(length, (int, float)):
        ...
        querry: str = ("SELECT film.length, AVG(payment.amount) FROM film " +
               "INNER JOIN inventory ON film.film_id = inventory.film_id " +
               "INNER JOIN rental ON inventory.inventory_id = rental.inventory_id " +
               "INNER JOIN payment ON rental.rental_id = payment.rental_id " +
               "WHERE film.length = {l} " + 
               "GROUP BY film.length").format(l=length)
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    if not isinstance(sum_min, (int, float)):
        ...
        querry: str = ("SELECT customer.first_name, customer.last_name, SUM(film.length) FROM customer " +
               "INNER JOIN rental ON customer.customer_id = rental.customer_id " +
               "INNER JOIN inventory ON rental.inventory_id = inventory.inventory_id " +
               "INNER JOIN film ON inventory.film_id = film.film_id " +
               "GROUP BY customer.first_name, customer.last_name " +
               "HAVING SUM(film.length) > {s_min} " + 
               "ORDER BY SUM(film.length), customer.last_name").format(s_min=sum_min)
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    if not isinstance(name, str):
        ...
        querry: str = ("SELECT cat.name AS category, AVG(film.length), SUM(film.length), MIN(film.length), MAX(film.length) FROM category cat " +
               "INNER JOIN film_category ON cat.category_id = film_category.category_id " +
               "INNER JOIN film ON film_category.film_id = film.film_id " +
               "WHERE cat.name = '{n}' " + 
               "GROUP BY cat.name").format(n=name)
